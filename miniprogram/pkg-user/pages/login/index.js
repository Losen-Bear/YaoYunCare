const { ensureOpenid, setStorage } = require('../../../api/request')
...
Page({
  ...
  doLogin() {
    ...
    ensureOpenid({ forceRefresh: true, showLoading: false })
      .then((openid) => {
        ...
        const finishLogin = (profile, syncProfile) => {
          ...
          if (openid) setStorage(STORAGE_KEYS.OPENID, openid)
          setStorage(STORAGE_KEYS.IS_LOGGED_IN, true)
          if (syncProfile) setStorage(STORAGE_KEYS.USER_PROFILE, userProfile)
